...
class FaceIdSecurityBloc
    extends Bloc<FaceIdSecurityEvent, FaceIdSecurityState> {
  FaceIdSecurityBloc({required FaceIdSecurityRepository repository})
  ...
  Future<void> _onStarted(
    ...
    emit(
      state.copyWith(status: FaceIdSecurityStatus.loading, errorMessage: null),
    );
    try {
      final content = await _repository.fetchContent();
      emit(
        state.copyWith(status: FaceIdSecurityStatus.ready, content: content),
      );
    } catch (e) {
      emit(
        state.copyWith(
          status: FaceIdSecurityStatus.failure,
