...
class CreatePasswordBloc
  ...
  CreatePasswordBloc({required this.repository})
  ...
  void _onPasswordChanged(
      PasswordChanged e, Emitter<CreatePasswordState> emit) {
    final next = state.copyWith(password: e.value, errorMessage: null);
    emit(_validate(next));
  }

  void _onConfirmChanged(
      ConfirmPasswordChanged e, Emitter<CreatePasswordState> emit) {
    final next = state.copyWith(confirmPassword: e.value, errorMessage: null);
    emit(_validate(next));
  }

  void _onTogglePassword(
      TogglePasswordVisibility e, Emitter<CreatePasswordState> emit) {
  ...
  Future<void> _onSubmit(
      SubmitCreatePassword event, Emitter<CreatePasswordState> emit) async {
    final pass = state.password.trim();
    final confirm = state.confirmPassword.trim();
    if (pass.length != 4) {
      AppUtils.showWarningToast('Password must be 4 characters long');
      return;
    }
    if (pass != confirm) {
      AppUtils.showWarningToast(
        "these passwords don't match. re-enter them to continue.",
      );
      return;
    }
    emit(state.copyWith(
        status: CreatePasswordStatus.submitting, errorMessage: null));
  ...
  CreatePasswordState _validate(CreatePasswordState s) {
    if (s.password.isEmpty && s.confirmPassword.isEmpty) {
      return s.copyWith(
          status: CreatePasswordStatus.initial, errorMessage: null);
    }
    return s.copyWith(
      status: s.canSubmit
          ? CreatePasswordStatus.valid
          : CreatePasswordStatus.invalid,
      errorMessage: null,
    );
  }
}
