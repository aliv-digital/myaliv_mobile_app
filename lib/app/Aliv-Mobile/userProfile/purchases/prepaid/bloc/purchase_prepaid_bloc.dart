...
class PurchasePrepaidBloc extends Bloc<PurchasePrepaidEvent, PurchasePrepaidState> {
  final PurchasePrepaidRepository repo;

  PurchasePrepaidBloc({required this.repo}):  super(PurchasePrepaidState.initial()) {
  ...
  Future<void> _onStarted(PurchasePrepaidStarted event,Emitter<PurchasePrepaidState> emit) async {
    emit(state.copyWith(status: PurchasePrepaidLoadStatus.loading, clearError: true));

    try {
      final items = await repo.fetchMenuItems();
      emit(state.copyWith(
  ...
  void _onNavConsumed(PurchasePrepaidNavigationConsumed event,Emitter<PurchasePrepaidState> emit) {
