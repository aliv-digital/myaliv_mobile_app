...
class HomeRoamingPromoResponse extends Equatable {
  const HomeRoamingPromoResponse({
  ...
  final HomeRoamingPromoCodeDefinition definition;
  final dynamic value;
  final int deviceId;
  ...
  bool get isApplied => promoCodeId > 0;

  factory HomeRoamingPromoResponse.fromDynamic(dynamic source) {
    if (source is String) {
      final decoded = jsonDecode(source);
      if (decoded is Map<String, dynamic>) {
        return HomeRoamingPromoResponse.fromJson(decoded);
      }
      if (decoded is Map) {
        return HomeRoamingPromoResponse.fromJson(_stringKeyMap(decoded));
      }
    }

    if (source is Map<String, dynamic>) {
      return HomeRoamingPromoResponse.fromJson(source);
    }

    if (source is Map) {
      return HomeRoamingPromoResponse.fromJson(_stringKeyMap(source));
    }

    throw const FormatException('Invalid promo response format.');
  }

  factory HomeRoamingPromoResponse.fromJson(Map<String, dynamic> json) {
    return HomeRoamingPromoResponse(
      promoCodeId: _asInt(json['PromoCodeID']),
      definition: HomeRoamingPromoCodeDefinition.fromJson(
        _asMap(json['Definition']),
      ),
      value: json['Value'],
...
class HomeRoamingPromoCodeDefinition extends Equatable {
  const HomeRoamingPromoCodeDefinition({
  ...
  factory HomeRoamingPromoCodeDefinition.fromJson(Map<String, dynamic> json) {
    return HomeRoamingPromoCodeDefinition(
