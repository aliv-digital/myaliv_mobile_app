/// Dedicated model for Monthly tab API data.
///
/// Goals:
...
class MonthlyPlanModel {
  ...
  // Nested fields
  final List<MonthlyPlanModel> availableBoltOns;
  final List<MonthlyPlanCapabilityModel> planCapabilities;
  final List<MonthlyPlanBucketModel> planBuckets;
  final MonthlyPlanDataRulesModel? dataRules;

  // Full raw object (optional, debug-only to reduce runtime memory).
  final Map<String, dynamic>? rawPayload;

  /// Build one `MonthlyPlanModel` from one API item map.
  ///
  /// This parser is intentionally defensive to avoid runtime crashes.
  factory MonthlyPlanModel.fromApiMap(
    ...
    return MonthlyPlanModel(
      ...
      availableBoltOns: _asMapList(safeMap['AvailableBoltOns'])
          .map(
            (Map<String, dynamic> addOnMap) => MonthlyPlanModel.fromApiMap(
              addOnMap,
      ...
      planCapabilities: _asMapList(safeMap['PlanCapabilities'])
          .map(MonthlyPlanCapabilityModel.fromApiMap)
          .toList(growable: false),
      planBuckets: _asMapList(safeMap['PlanBuckets'])
          .map(MonthlyPlanBucketModel.fromApiMap)
          .toList(growable: false),
      ...
      dataRules: _asMapOrNull(safeMap['DataRules']) == null
          ? null
          : MonthlyPlanDataRulesModel.fromApiMap(
  ...
  /// Strict Monthly filter rule from requirement.
  /// - PlanType = P
  /// - Frequency = M
  bool get isStrictMonthlyPlan => planType == 'P' && frequency == 'M';
...
class MonthlyPlanCapabilityModel {
  const MonthlyPlanCapabilityModel({
  ...
  factory MonthlyPlanCapabilityModel.fromApiMap(Map<String, dynamic> map) {
    return MonthlyPlanCapabilityModel(
      planCapabilityName: MonthlyPlanModel._asString(map['PlanCapabilityName']),
      planCapabilityType: MonthlyPlanModel._asString(map['PlanCapabilityType']),
    );
  }
...
class MonthlyPlanBucketModel {
  const MonthlyPlanBucketModel({
    required this.name,
  ...
  factory MonthlyPlanBucketModel.fromApiMap(Map<String, dynamic> map) {
    return MonthlyPlanBucketModel(
      name: MonthlyPlanModel._asString(map['Name']),
      amount: MonthlyPlanModel._asDouble(map['Amount']),
      unit: MonthlyPlanModel._asString(map['Unit']),
      bucketOrder: MonthlyPlanModel._asString(map['BucketOrder']),
      suppress: MonthlyPlanModel._asBool(map['Suppress']),
      unlimited: MonthlyPlanModel._asBool(map['Unlimited']),
      bucketUnit: MonthlyPlanModel._asString(map['BucketUnit']),
    );
  }
...
class MonthlyPlanDataRulesModel {
  const MonthlyPlanDataRulesModel({
    required this.planId,
  ...
  factory MonthlyPlanDataRulesModel.fromApiMap(Map<String, dynamic> map) {
    return MonthlyPlanDataRulesModel(
      planId: MonthlyPlanModel._asInt(map['PlanId']),
      requireAltContactPhone:
          MonthlyPlanModel._asBool(map['RequireAltContactPhone']),
    );
  }
