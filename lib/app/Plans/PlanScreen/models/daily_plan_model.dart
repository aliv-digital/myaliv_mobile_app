/// Dedicated model for Daily tab API data.
///
/// Goals:
...
class DailyPlanModel {
  ...
  // Nested fields
  final List<DailyPlanModel> availableBoltOns;
  final List<DailyPlanCapabilityModel> planCapabilities;
  final List<DailyPlanBucketModel> planBuckets;
  final DailyPlanDataRulesModel? dataRules;

  // Full raw object (optional, debug-only to reduce runtime memory).
  final Map<String, dynamic>? rawPayload;

  /// Build one `DailyPlanModel` from one API item map.
  ///
  /// This parser is intentionally defensive to avoid runtime crashes.
  factory DailyPlanModel.fromApiMap(
    ...
    return DailyPlanModel(
      ...
      availableBoltOns: _asMapList(safeMap['AvailableBoltOns'])
          .map(
            (Map<String, dynamic> addOnMap) => DailyPlanModel.fromApiMap(
              addOnMap,
      ...
      planCapabilities: _asMapList(safeMap['PlanCapabilities'])
          .map(DailyPlanCapabilityModel.fromApiMap)
          .toList(growable: false),
      planBuckets: _asMapList(safeMap['PlanBuckets'])
          .map(DailyPlanBucketModel.fromApiMap)
          .toList(growable: false),
      ...
      dataRules: _asMapOrNull(safeMap['DataRules']) == null
          ? null
          : DailyPlanDataRulesModel.fromApiMap(
  ...
  /// Strict Daily filter rule from requirement.
  /// - PlanType = P
  /// - Frequency = D
  bool get isStrictDailyPlan => planType == 'P' && frequency == 'D';
...
class DailyPlanCapabilityModel {
  const DailyPlanCapabilityModel({
  ...
  factory DailyPlanCapabilityModel.fromApiMap(Map<String, dynamic> map) {
    return DailyPlanCapabilityModel(
      planCapabilityName: DailyPlanModel._asString(map['PlanCapabilityName']),
      planCapabilityType: DailyPlanModel._asString(map['PlanCapabilityType']),
    );
  }
...
class DailyPlanBucketModel {
  const DailyPlanBucketModel({
    required this.name,
  ...
  factory DailyPlanBucketModel.fromApiMap(Map<String, dynamic> map) {
    return DailyPlanBucketModel(
      name: DailyPlanModel._asString(map['Name']),
      amount: DailyPlanModel._asDouble(map['Amount']),
      unit: DailyPlanModel._asString(map['Unit']),
      bucketOrder: DailyPlanModel._asString(map['BucketOrder']),
      suppress: DailyPlanModel._asBool(map['Suppress']),
      unlimited: DailyPlanModel._asBool(map['Unlimited']),
      bucketUnit: DailyPlanModel._asString(map['BucketUnit']),
    );
  }
...
class DailyPlanDataRulesModel {
  const DailyPlanDataRulesModel({
    required this.planId,
  ...
  factory DailyPlanDataRulesModel.fromApiMap(Map<String, dynamic> map) {
    return DailyPlanDataRulesModel(
      planId: DailyPlanModel._asInt(map['PlanId']),
      requireAltContactPhone:
          DailyPlanModel._asBool(map['RequireAltContactPhone']),
    );
  }
