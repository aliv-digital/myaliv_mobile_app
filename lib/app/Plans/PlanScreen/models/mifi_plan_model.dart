/// Dedicated model for MiFi tab API data.
///
/// Goals:
...
class MifiPlanModel {
  ...
  // Nested fields
  final List<MifiPlanModel> availableBoltOns;
  final List<MifiPlanCapabilityModel> planCapabilities;
  final List<MifiPlanBucketModel> planBuckets;
  final MifiPlanDataRulesModel? dataRules;

  // Full raw object (optional, debug-only to reduce runtime memory).
  final Map<String, dynamic>? rawPayload;

  /// Build one `MifiPlanModel` from one API item map.
  ///
  /// This parser is intentionally defensive to avoid runtime crashes.
  factory MifiPlanModel.fromApiMap(
    Map<String, dynamic> map, {
    bool includeRawPayload = false,
  }) {
    final Map<String, dynamic> safeMap = _toStringKeyedMap(map);

    return MifiPlanModel(
      ...
      availableBoltOns: _asMapList(safeMap['AvailableBoltOns'])
          .map(
            (Map<String, dynamic> addOnMap) => MifiPlanModel.fromApiMap(
              addOnMap,
      ...
      planCapabilities: _asMapList(safeMap['PlanCapabilities'])
          .map(MifiPlanCapabilityModel.fromApiMap)
          .toList(growable: false),
      planBuckets: _asMapList(safeMap['PlanBuckets'])
          .map(MifiPlanBucketModel.fromApiMap)
          .toList(growable: false),
      ...
      dataRules: _asMapOrNull(safeMap['DataRules']) == null
          ? null
          : MifiPlanDataRulesModel.fromApiMap(
  ...
  /// Strict MiFi filter rule from requirement.
  /// - PlanType = P
  /// - PlanGroup = mifi (30 day)
  bool get isStrictMifiPlan {
    return planType.trim().toUpperCase() == 'P' &&
        planGroup.trim().toLowerCase() == 'mifi (30 day)';
  }
...
class MifiPlanCapabilityModel {
  const MifiPlanCapabilityModel({
  ...
  factory MifiPlanCapabilityModel.fromApiMap(Map<String, dynamic> map) {
    return MifiPlanCapabilityModel(
      planCapabilityName: MifiPlanModel._asString(map['PlanCapabilityName']),
      planCapabilityType: MifiPlanModel._asString(map['PlanCapabilityType']),
    );
  }
...
class MifiPlanBucketModel {
  const MifiPlanBucketModel({
    required this.name,
  ...
  factory MifiPlanBucketModel.fromApiMap(Map<String, dynamic> map) {
    return MifiPlanBucketModel(
      name: MifiPlanModel._asString(map['Name']),
      amount: MifiPlanModel._asDouble(map['Amount']),
      unit: MifiPlanModel._asString(map['Unit']),
      bucketOrder: MifiPlanModel._asString(map['BucketOrder']),
      suppress: MifiPlanModel._asBool(map['Suppress']),
      unlimited: MifiPlanModel._asBool(map['Unlimited']),
      bucketUnit: MifiPlanModel._asString(map['BucketUnit']),
    );
  }
...
class MifiPlanDataRulesModel {
  const MifiPlanDataRulesModel({
    required this.planId,
  ...
  factory MifiPlanDataRulesModel.fromApiMap(Map<String, dynamic> map) {
    return MifiPlanDataRulesModel(
      planId: MifiPlanModel._asInt(map['PlanId']),
      requireAltContactPhone:
          MifiPlanModel._asBool(map['RequireAltContactPhone']),
    );
  }
