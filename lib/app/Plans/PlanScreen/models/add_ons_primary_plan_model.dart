...
class AddOnsPrimaryPlanModel {
  ...
  /// Build one `AddOnsPrimaryPlanModel` from one bundles API item map.
  ///
  /// This parser is intentionally defensive because bundles payloads contain
  /// mixed types, optional values, and nested arrays.
  ...
  /// True when this item is a real primary plan from `PrimaryPlans`.
  bool get isPrimaryPlan => planType.trim().toUpperCase() == 'P';

  /// True when this item is an add-on entry nested under `AvailableBoltOns`.
  ...
  /// Safe date parsing helper for UI and repository sorting logic.
  /// Returns `null` when source date is empty or invalid.
...
/// Plan capability block from bundles API payload.
...
/// One bucket inside a bundles API plan payload.
...
/// DataRules nested object from bundles API payload.
