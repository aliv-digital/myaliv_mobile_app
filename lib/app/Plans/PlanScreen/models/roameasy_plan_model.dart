/// Dedicated model for RoamEasy tab API data.
///
/// Goals:
...
class RoamEasyPlanModel {
  const RoamEasyPlanModel({
  ...
  // Nested fields
  final List<RoamEasyPlanModel> availableBoltOns;
  final List<RoamEasyPlanCapabilityModel> planCapabilities;
  final List<RoamEasyPlanBucketModel> planBuckets;
  final RoamEasyPlanDataRulesModel? dataRules;

  // Full raw object (optional, debug-only to reduce runtime memory).
  final Map<String, dynamic>? rawPayload;

  /// Build one `RoamEasyPlanModel` from one API item map.
  ///
  /// This parser is intentionally defensive to avoid runtime crashes.
  factory RoamEasyPlanModel.fromApiMap(
    Map<String, dynamic> map, {
    bool includeRawPayload = false,
  }) {
    final Map<String, dynamic> safeMap = _toStringKeyedMap(map);

    return RoamEasyPlanModel(
      ...
      availableBoltOns: _asMapList(safeMap['AvailableBoltOns'])
          .map(
            (Map<String, dynamic> addOnMap) => RoamEasyPlanModel.fromApiMap(
              addOnMap,
      ...
      planCapabilities: _asMapList(safeMap['PlanCapabilities'])
          .map(RoamEasyPlanCapabilityModel.fromApiMap)
          .toList(growable: false),
      planBuckets: _asMapList(safeMap['PlanBuckets'])
          .map(RoamEasyPlanBucketModel.fromApiMap)
          .toList(growable: false),
      ...
      dataRules: _asMapOrNull(safeMap['DataRules']) == null
          ? null
          : RoamEasyPlanDataRulesModel.fromApiMap(
  ...
  /// Strict RoamEasy filter rule from requirement.
  /// - PlanType = A
  /// - PlanGroup = roameasy
  bool get isStrictRoamEasyPlan {
    return planType.trim().toUpperCase() == 'A' &&
        planGroup.trim().toLowerCase() == 'roameasy';
  }
...
class RoamEasyPlanCapabilityModel {
  const RoamEasyPlanCapabilityModel({
  ...
  factory RoamEasyPlanCapabilityModel.fromApiMap(Map<String, dynamic> map) {
    return RoamEasyPlanCapabilityModel(
      planCapabilityName:
          RoamEasyPlanModel._asString(map['PlanCapabilityName']),
      planCapabilityType:
          RoamEasyPlanModel._asString(map['PlanCapabilityType']),
    );
  }
...
class RoamEasyPlanBucketModel {
  const RoamEasyPlanBucketModel({
    required this.name,
  ...
  factory RoamEasyPlanBucketModel.fromApiMap(Map<String, dynamic> map) {
    return RoamEasyPlanBucketModel(
      name: RoamEasyPlanModel._asString(map['Name']),
      amount: RoamEasyPlanModel._asDouble(map['Amount']),
      unit: RoamEasyPlanModel._asString(map['Unit']),
      bucketOrder: RoamEasyPlanModel._asString(map['BucketOrder']),
      suppress: RoamEasyPlanModel._asBool(map['Suppress']),
      unlimited: RoamEasyPlanModel._asBool(map['Unlimited']),
      bucketUnit: RoamEasyPlanModel._asString(map['BucketUnit']),
    );
  }
...
class RoamEasyPlanDataRulesModel {
  const RoamEasyPlanDataRulesModel({
    required this.planId,
  ...
  factory RoamEasyPlanDataRulesModel.fromApiMap(Map<String, dynamic> map) {
    return RoamEasyPlanDataRulesModel(
      planId: RoamEasyPlanModel._asInt(map['PlanId']),
      requireAltContactPhone:
          RoamEasyPlanModel._asBool(map['RequireAltContactPhone']),
    );
  }
